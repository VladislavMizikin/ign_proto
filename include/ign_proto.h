/**
 * @file ign_proto.h
 * @author Мизикин Владислав
 * @date 26.02.2026
 * @brief Заголовочный файл протокола IGN.
 *
 * Содержит определения типов данных, структур, конфигураций
 * и прототипы функций для работы с протоколом IGN.
 */

/* Directive to prevent recursive inclusion ----------------------------------*/

#pragma once

/* Includes ------------------------------------------------------------------*/

#include <stdint.h>
#include <stddef.h>
#include "ign_proto_errors.h"
#include "ign_proto_cmdspace.h"

/* Exported types ----------------------------------------------------------*/

#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Структура метрик протокола IGN
 *
 * Содержит статистическую информацию о работе протокола,
 * включая счётчики соединений и приёма данных.
 */
typedef struct
{
	uint32_t host_unix_sec;       ///< Текущее Unix-время хоста (в секундах)
	int16_t host_tz_min;          ///< Смещение часового пояса хоста относительно UTC (в минутах)

	uint32_t install_attempts;    ///< Количество попыток установки соединения
	uint32_t install_success;     ///< Количество успешных установок соединения
	uint32_t install_fail;        ///< Количество неудачных установок соединения

	uint32_t rx_bytes;            ///< Общее количество принятых байтов
	uint32_t rx_frames_ok;        ///< Количество корректно принятых фреймов
	uint32_t rx_frames_crc_err;   ///< Количество фреймов с ошибкой CRC
} ign_proto_metrics_t;

/**
 * @brief Тип функции передачи байтов
 *
 * Используется для отправки данных транспортным уровнем.
 *
 * @param data Указатель на буфер с передаваемыми данными
 * @param len Количество байтов для передачи
 * @return 0 при успешной передаче, ненулевое значение при ошибке
 */
typedef int (*ign_proto_tx_bytes_fn)(const uint8_t *data, uint32_t len);

/**
 * @brief Тип функции инициализации (сброса) расчёта CRC
 *
 * Должна подготавливать внутреннее состояние CRC к новому расчёту.
 */
typedef void (*ign_proto_crc_init_fn)(void);

/**
 * @brief Тип функции обновления CRC
 *
 * Добавляет данные в текущий расчёт CRC.
 *
 * @param data Указатель на данные
 * @param len Длина данных в байтах
 */
typedef void (*ign_proto_crc_update_fn)(const void *data, uint32_t len);

/**
 * @brief Тип функции завершения расчёта CRC
 *
 * @return Итоговое значение CRC32
 */
typedef uint32_t (*ign_proto_crc_final_fn)(void);

/**
 * @brief Тип функции диспетчеризации команд
 *
 * Вызывается при успешном разборе входящего фрейма.
 *
 * @param cmd Код команды
 * @param request_id Идентификатор транзакции команды
 * @param payload Указатель на полезные данные (может быть NULL, если payload_len == 0)
 * @param payload_len Длина полезных данных в байтах
 * @return Статус выполнения команды (IGN_PROTO_OK при успехе)
 */
typedef ign_proto_status_t (*ign_proto_cmd_dispatch_fn)(
		uint8_t cmd, uint16_t request_id, const uint8_t *payload, uint16_t payload_len);

/**
 * @brief Конфигурация протокола IGN
 *
 * Содержит набор пользовательских функций,
 * необходимых для работы протокола.
 */
typedef struct
{
	ign_proto_tx_bytes_fn tx_bytes;       ///< Функция передачи байтов
	ign_proto_crc_init_fn crc_init;       ///< Функция инициализации расчёта CRC
	ign_proto_crc_update_fn crc_update;   ///< Функция обновления CRC
	ign_proto_crc_final_fn crc_final;     ///< Функция завершения расчёта CRC
	ign_proto_cmd_dispatch_fn dispatch;   ///< Функция обработки (диспетчеризации) команд
} ign_proto_cfg_t;

/**
 * @brief Тип callback-функции приёма байтов
 *
 * Используется для передачи входящих данных в стек протокола.
 *
 * @param data Указатель на массив входящих байтов
 * @param size Количество байтов в массиве
 */
typedef void (*ign_proto_rx_cb_t)(const uint8_t *data, uint32_t size);

/**
 * @brief Основной контекст протокола IGN
 *
 * Содержит текущее состояние протокола, конфигурацию,
 * буферы и служебные данные парсера.
 */
typedef struct ign_proto
{
	ign_proto_cfg_t cfg;            ///< Конфигурация протокола

	uint8_t *payload_buf;           ///< Буфер для хранения полезных данных
	uint16_t payload_buf_sz;        ///< Размер буфера полезных данных

	/* parsing state */
	uint8_t st;                     ///< Текущее состояние парсера (например, ST_SOF0, ST_SOF1 и др.)
	uint16_t payload_len;           ///< Ожидаемая длина полезной нагрузки
	uint16_t payload_pos;           ///< Текущая позиция записи в payload_buf
	uint8_t cmd;                    ///< Код текущей обрабатываемой команды
	uint16_t request_id;             ///< Идентификатор текущей транзакции

	uint8_t crc_bytes[4];           ///< Буфер для хранения принятых байтов CRC32
	uint8_t crc_pos;                ///< Текущая позиция в буфере CRC

	ign_proto_metrics_t metrics;    ///< Метрики протокола

	uint8_t enable_core_cmds;       ///< Флаг обработки базовых команд (1 — включено, 0 — отключено)
} ign_proto_t;

/* Exported functions prototypes ---------------------------------------------*/

/**
 * @brief Инициализация контекста протокола IGN
 *
 * Выполняет инициализацию структуры протокола и возвращает
 * callback-функцию для передачи входящих данных.
 *
 * @param p Указатель на структуру контекста протокола
 * @param cfg Указатель на конфигурацию протокола
 * @param payload_buf Указатель на буфер для полезных данных
 * @param payload_buf_sz Размер буфера полезных данных в байтах
 *
 * @return Callback-функция приёма байтов (ign_proto_rx_cb_t) или NULL при ошибке
 *
 * @details
 * После успешного вызова функции необходимо передавать входящие данные
 * в возвращённый callback для обработки протоколом.
 */
ign_proto_rx_cb_t ign_proto_init(ign_proto_t *p,
								 const ign_proto_cfg_t *cfg,
								 uint8_t *payload_buf,
								 uint16_t payload_buf_sz);

/**
 * @brief Получение метрик протокола
 *
 * @param p Указатель на контекст протокола
 * @return Указатель на структуру метрик или NULL, если p == NULL
 */
const ign_proto_metrics_t *ign_proto_metrics_get(const ign_proto_t *p);

/**
 * @brief Увеличение счётчика попыток установки соединения
 *
 * @param p Указатель на контекст протокола
 */
void ign_proto_metrics_install_attempt(ign_proto_t *p);

/**
 * @brief Фиксация результата установки соединения
 *
 * @param p Указатель на контекст протокола
 * @param success Признак успеха (ненулевое значение — успех, 0 — ошибка)
 */
void ign_proto_metrics_install_result(ign_proto_t *p, int success);

/**
 * @brief Отправка подтверждения (ACK)
 *
 * @param p Указатель на контекст протокола
 * @param orig_cmd Код команды, для которой отправляется подтверждение
 * @param request_id Идентификатор подтверждаемой транзакции
 * @return Статус выполнения (IGN_PROTO_OK при успехе)
 */
ign_proto_status_t ign_proto_send_ack(ign_proto_t *p, uint8_t orig_cmd, uint16_t request_id);

/**
 * @brief Отправка отрицательного подтверждения (NACK)
 *
 * @param p Указатель на контекст протокола
 * @param orig_cmd Код команды, вызвавшей ошибку
 * @param nack_code Код ошибки (определяется протоколом)
 * @param request_id Идентификатор отклоняемой транзакции
 * @return Статус выполнения (IGN_PROTO_OK при успехе)
 */
ign_proto_status_t ign_proto_send_nack(ign_proto_t *p, uint8_t orig_cmd, uint8_t nack_code, uint16_t request_id);

/**
 * @brief Включение или отключение обработки базовых команд
 *
 * @param p Указатель на контекст протокола
 * @param enable Флаг включения (ненулевое значение — включить, 0 — отключить)
 */
void ign_proto_enable_core(ign_proto_t *p, int enable);

#ifdef __cplusplus
}
#endif